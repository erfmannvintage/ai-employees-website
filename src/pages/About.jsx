    ...
    <div>
      <section className="py-20 lg:py-28 section-cream halftone">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-pop-red font-extrabold uppercase tracking-[0.2em] text-sm mb-3">Our Story</p>
          <h1 className="font-black text-5xl sm:text-6xl lg:text-7xl uppercase text-ink mb-4">The Team Behind<br /><span className="text-pop-red">AI Employees</span></h1>
          <p className="text-ink-muted text-lg max-w-xl mx-auto">Every business deserves a world-class team — regardless of budget.</p>
        </div>
      </section>
      ...
      <section className="py-20 section-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-black text-3xl sm:text-4xl lg:text-5xl uppercase text-ink mb-6">You Want to Scale, But...</h2>
          ...
          <h3 className="font-black text-2xl sm:text-3xl uppercase text-ink mt-10">You're Wearing Too Many Hats:</h3>
          <p className="text-ink-muted text-lg mt-2">CEO, marketer, sales rep, support agent, accountant...</p>
        </div>
      </section>
      ...
      <section className="py-20 section-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <h2 className="font-black text-4xl sm:text-5xl uppercase text-ink">Your <span className="text-pop-red">11 Employees</span></h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {employees.map((emp) => (
              <div key={emp.name} className="comic-panel rounded-xl overflow-hidden">
                ...
                <div className="p-5">
                  <h3 className="font-black text-ink uppercase text-lg">{emp.name}</h3>
                  <p className="text-pop-red text-xs font-bold uppercase tracking-wide mb-3">{emp.role}</p>
                  <p className="text-ink-faint text-sm leading-relaxed italic">"{emp.description}"</p>
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {emp.skills.slice(0, 3).map((s) => (
                      <span key={s} className="px-2 py-0.5 text-[10px] font-bold text-pop-red/70 bg-pop-red/[0.06] border border-pop-red/10 rounded">{s}</span>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.l} className="comic-panel rounded-xl p-6 text-center">
                <div className="font-black text-4xl text-pop-red">{s.v}</div>
                <div className="text-xs font-bold text-ink-faint uppercase tracking-wide mt-2">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
