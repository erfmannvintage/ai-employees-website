    ...
    <div>
      <section className="py-20 lg:py-28 section-cream halftone">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-pop-red font-extrabold uppercase tracking-[0.2em] text-sm mb-3">Support</p>
          <h1 className="font-black text-5xl sm:text-6xl lg:text-7xl uppercase text-ink mb-4">Help <span className="text-pop-red">Centre</span></h1>
          <p className="text-ink-muted mt-4">Can't find your answer? <Link to="/contact" className="text-pop-red font-bold hover:underline">Contact us</Link>.</p>
        </div>
      </section>
      ...
      <section className="py-20 section-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          {cats.map((cat) => (
            <div key={cat.name}>
              <h2 className="font-black text-2xl text-ink uppercase mb-5 flex items-center gap-3"><span className="text-2xl">{cat.icon}</span>{cat.name}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {cat.qs.map((faq, i) => {
                  const id = `${cat.name}-${i}`
                  return (
                    <div key={id} onClick={() => setOpen(open === id ? null : id)} className={`cursor-pointer rounded-xl border-3 transition-all ${open === id ? 'border-neon bg-neon text-black' : 'border-light-200 bg-white text-ink hover:border-ink-ghost'}`}>
                      <div className="px-5 py-4">
                        <h3 className="font-bold text-sm">{faq.q}</h3>
                        {open === id && <p className="text-xs mt-2 leading-relaxed text-black/70">{faq.a}</p>}
                      </div>
                    </div>
                  )
                })}
      ...
      <section className="py-16 section-cream">
        <div className="max-w-lg mx-auto px-4 text-center">
          <div className="comic-panel rounded-xl p-8">
            <h2 className="font-black text-2xl text-ink uppercase mb-3">Still Have Questions?</h2>
